var ps = {
    ...
    o: {
        ...
        hints: {
            ...
            'img.settings' : 'Click here to open the settings dialog',
            '#openPhotoConfig button' : 'Click here to open the photo config dialog'
        },
    ...
    fn: {
        ...
        displayHint: function ( selector ){
            var $hint = $('<div></div>', { class : 'hint', text : ps.o.hints[selector] });
            $(selector).after($hint);
        },
        ...
        notify: function (message, type, time, callback, stack) {
            ...
            if (time === 0) $('#globalNotification .inner').last().append($('<button></button>', {class:'closeButton', text: 'x'}));
        ...
        refreshPhotoConfigData: function (categoryToScroll, callback) {
            ...
            $.getJSON('php/launchconfig.php', function (data) {
                ...
                if (typeof callback === 'function') callback();
                if (typeof categoryToScroll === 'string') $('#dialog-config').scrollTop($('#' + categoryToScroll + '_container').position().top);
        ...
        sortableConfigPhotos_toggleOff: function () {
            ...
            $('.categoryContainer').each(function (index) {
                $(this).find('.sortableContainer').sortable('disable');
            });
        },
        sortableConfigPhotos_toggleOn: function () {
            $('.categoryContainer').each(function (index) {
                $(this).find('.sortableContainer').sortable('enable');
            });
