...
ps.events.contentPaneMouseover = function (e) {
    ...
    if (ps.o.categoryData[this.id].defaultView == 'slider') ps.fn.notify('Click to open the Slider view', '', 0);
...
//$(document).on('blur', '.contentpane', ps.events.contentPaneMouseleave);
//$(document).on('focus', '.contentpane', ps.events.contentPaneMouseover);
...
$("#setHue").slider({
    ...
    slide: function (event, ui) {
        ...
        var oldcolor = ps.o.config[whichelement] || ps.o.categoryData[ps.fn.urlvars().cat][whichelement];
...
$(document).on('click', '#toggleCaptions>button', function () {
    var cat = ps.fn.urlvars().cat;
    if (ps.o.categoryData[cat].toggleCaption != 'on') ps.fn.toggleCaptionDisplay(cat, 'on');
    else ps.fn.toggleCaptionDisplay(cat, 'off');
